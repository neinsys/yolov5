...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    im = []
    for _ in range(nStream):
        imBufTemp = torch.ByteTensor(3,384,640).to(device)
        if half:
            imTemp = torch.HalfTensor(3,384,640).to(device)
        else:
            imTemp = torch.FloatTensor(3,384,640).to(device)
        if len(imTemp.shape) == 3:
            imTemp = imTemp[None]  # expand for batch dim
        imBuf.append(imBufTemp)
        im.append(imTemp)
    ...
    while True:
        ...
        for idx, item in enumerate(data):
            with torch.cuda.stream(streams[idx]):
                ...
                pred = model(im[idx], augment=augment, visualize=visualize)
                result.append(pred)
        t3 = time_sync()
        dt[1] += t3 - t2
        ...
        for idx, item in enumerate(data):
            with torch.cuda.stream(streams[idx]):
                path, _, im0s, vid_cap, s = item
                for i, det in enumerate(result[idx]):  # per image
                    seen += 1
                    ...
                    s += '%gx%g ' % im[idx].shape[2:]  # print string
                    gn = torch.tensor(im0.shape)[[1, 0, 1, 0]]  # normalization gain whwh
                    imc = im0.copy() if save_crop else im0  # for save_crop
                    annotator = Annotator(im0, line_width=line_thickness, example=str(names))
                    if len(det):
                        # Rescale boxes from img_size to im0 size
                        det[:, :4] = scale_coords(im[idx].shape[2:], det[:, :4], im0.shape).round()
