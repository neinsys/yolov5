...
def non_max_suppression_custom(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                               labels=(), max_det=300):
    """Runs custom Non-Maximum Suppression (NMS) on inference results
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        MAX_MEM = 100
        i = torch.IntTensor(MAX_MEM).to(prediction.device)
        check = torch.IntTensor(MAX_MEM).to(prediction.device)
        score_temp = torch.FloatTensor(scores.size()).to(prediction.device)
        sz = custom_cuda.nms(boxes,scores,iou_thres,i,check,score_temp)
        i = i.long()
        #i = torchvision.ops.nms(boxes, scores, iou_thres)  # NMS
        if i.shape[0] > max_det:  # limit detections
            i = i[:max_det]
